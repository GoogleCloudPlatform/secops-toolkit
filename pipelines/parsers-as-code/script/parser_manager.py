...
from models import (
    ...
    ParserValidationStatus, ValidationError, ParserError, APIError, ParserType,
    ParserDeploymentPlan
)
from config import (
    PARSERS_ROOT_DIR, PARSER_CONFIG_FILENAME, PARSER_EXT_CONFIG_FILENAME,
    LOGS_FOLDER_NAME, EVENTS_FOLDER_NAME, PARSER_YAML_FILENAME
)
...
class ParserManager:
    ...
    def discover_local_configs(self) -> List[LogTypeConfig]:
        ...
        for item in sorted(os.listdir(PARSERS_ROOT_DIR)):
            ...
            if os.path.isfile(parser_yaml_path):
                try:
                    with open(parser_yaml_path, 'r', encoding='utf-8') as f:
                        ...
                        if yaml_content.get("parser", {}).get("type") == ParserType.PREBUILT.value:
                            config.parser_type = ParserType.PREBUILT
                        else:
                            config.parser_type = ParserType.CUSTOM

                        # Override filenames if specified in YAML
                        if "parser" in yaml_content and "cbn" in yaml_content["parser"]:
                            parser_conf_filename = yaml_content["parser"]["cbn"]
                        
                        if "parser_extension" in yaml_content and "cbn_snippet" in yaml_content["parser_extension"]:
                            parser_ext_conf_filename = yaml_content["parser_extension"]["cbn_snippet"]

                except yaml.YAMLError as e:
                    LOGGER.warning(f"[{item}] Failed to parse {PARSER_YAML_FILENAME}: {e}. Falling back to defaults.")
            ...
            parser_ext_conf_path = os.path.join(parser_dir_path, parser_ext_conf_filename)
    ...
    def _get_active_content(self, log_type: str, is_extension: bool, parser_type: ParserType = ParserType.CUSTOM) -> str | None:
        ...
        try:
            if is_extension:
                ...
                for parser in parsers:
                    if parser.get("state") == ParserState.ACTIVE.value and "cbn" in parser:
                        if parser_type == ParserType.CUSTOM and parser.get("type") == ParserType.CUSTOM.value:
                            return base64.b64decode(parser["cbn"]).decode('utf-8')
                        elif parser_type == ParserType.PREBUILT and parser.get("type") != ParserType.CUSTOM.value:
                            return base64.b64decode(parser["cbn"]).decode('utf-8')
    ...
    def plan_deployment(self) -> dict[str, ParserDeploymentPlan]:
        ...
        for config in all_configs:
            ...
            if config.parser:
                active_parser = self._get_active_content(config.log_type, is_extension=False, parser_type=config.parser_type)
                
                if config.parser_type == ParserType.PREBUILT:
                    if not active_parser or active_parser.strip() != config.parser.strip():
                        try:
                            parsers = self.client.list_parsers(config.log_type)
                            found_rc = False
                            for p in parsers:
                                if (p.get("type") == ParserType.PREBUILT.value and 
                                    p.get("state") != ParserState.ACTIVE.value and 
                                    p.get("releaseStage") == ParserState.RELEASE_CANDIDATE.value):
                                    
                                    rc_content = base64.b64decode(p.get("cbn", "")).decode('utf-8')
                                    if rc_content.strip() == config.parser.strip():
                                        LOGGER.info(f"[{config.log_type}] Local code matches pending Release Candidate {p.get('name')}. Operation: RELEASE.")
                                        plan_op.parser_operation = Operation.RELEASE
                                        found_rc = True
                                        break
                            
                            if not found_rc:
                                LOGGER.warning(f"[{config.log_type}] Local code differs from active parser and no matching Release Candidate found. Validation only (UPDATE).")
                                plan_op.parser_operation = Operation.UPDATE
                        except Exception as e:
                            LOGGER.warning(f"[{config.log_type}] Failed to check release candidates: {e}. Defaulting to UPDATE.")
                            plan_op.parser_operation = Operation.UPDATE
                    else:
            ...
            if (plan_op.parser_operation != Operation.NONE or
            ...
            needs_comparison = (
                plan_op.parser_operation in [Operation.UPDATE, Operation.RELEASE] or
                plan_op.parser_ext_operation in [Operation.CREATE, Operation.UPDATE]
            )
            
            if needs_comparison:
                try:
                    from compare import ParserComparator
                    comparator = ParserComparator(config.log_type, client=self.client)
                    
                    LOGGER.info(f"[{config.log_type}] Generating UDM comparison report...")
                    ...
                    LOGGER.error(f"[{config.log_type}] Failed to generate comparison report: {e}")
                    plan_op.comparison_report = f"Failed to generate comparison report: {e}"

            plan[config.log_type] = plan_op
        return plan

    def execute_deployment(self, plan: dict[str, ParserDeploymentPlan]) -> list:
        ...
        for log_type, details in plan.items():
            ...
            # Submit parser
            if details.parser_operation in [Operation.CREATE, Operation.UPDATE, Operation.RELEASE]:
                if details.config.parser_type == ParserType.PREBUILT:
                    LOGGER.info(f"[{log_type}] PREBUILT parser detected. Skipping submission.")
    ...
    def verify_submissions(self, submitted_info: list, plan: dict[str, ParserDeploymentPlan]):
                    ...
                    for p in parsers:
                        if (p.get("type") == ParserType.PREBUILT.value and 
                            ...
                            if rc_content.strip() == config.parser.strip():
                                ...
                                LOGGER.info(f"[{config.log_type}] Found matching Release Candidate: {rc_id}. Activating...")
                                # Using activate_parser as it allows activating a specific parser ID (which this RC is)
                                self.client.activate_release_candidate_parser(config.log_type, rc_id)
                                activated_count += 1
                                break
                    else:
                        LOGGER.warning(f"[{config.log_type}] No matching Release Candidate found for local content. Skipping activation.")

                except Exception as e:
                    LOGGER.error(f"[{config.log_type}] Failed to activate release candidate: {e}")

            else:
                 # Handle CUSTOM parser activation
                parsers = self.client.list_parsers(config.log_type)
                for p in parsers:
                    if (p.get("type") == ParserType.CUSTOM.value and
                            p.get("state") != ParserState.ACTIVE.value and
    ...
    def _validate_parser_events(self, config: LogTypeConfig):
        ...
        if not os.path.isdir(logs_subfolder):
            LOGGER.warning(f"[{config.log_type}] Missing '{LOGS_FOLDER_NAME}/' folder. Skipping event validation.")
            return

        log_files = [f for f in sorted(os.listdir(logs_subfolder)) if os.path.isfile(os.path.join(logs_subfolder, f))]
        if not log_files:
            LOGGER.warning(f"[{config.log_type}] No log files found in '{LOGS_FOLDER_NAME}/'. Skipping event validation.")
            return

        if not os.path.isdir(events_subfolder):
            os.makedirs(events_subfolder, exist_ok=True)
            LOGGER.info(f"[{config.log_type}] Created missing '{EVENTS_FOLDER_NAME}/' folder.")
        ...
        for log_filename in log_files:
            ...
            processed_generated_events = process_data_for_dump(generated_events)
            ...
            if os.path.exists(event_filepath):
                with open(event_filepath, "r", encoding='utf-8') as ef:
                    try:
                        ...
                        LOGGER.warning(f"Could not parse YAML from {event_filepath}. It will be overwritten.")
            ...
            with tempfile.NamedTemporaryFile("w+", delete=False, suffix=".yaml") as f_new, \
                ...
                yaml.dump(process_data_for_dump(local_events), f_exp, sort_keys=True)

            try:
                diffs = compare_yaml_files(temp_exp_path, temp_new_path, ["timestamp", "Timestamp", "etag"])
                if diffs:
                    LOGGER.warning(f"[{config.log_type}] Differences found for {log_filename}. Updating local events file: {event_filepath}")
                    with open(event_filepath, "w", encoding='utf-8') as ef:
                        yaml.dump(processed_generated_events, ef, sort_keys=True)
            finally:
    ...
    def pull_all_parsers(self):
        ...
        try:
            ...
            all_parsers = []
            response = self.client.list_parsers(log_type='-', page_size=1000, page_token=None)
            ...
            LOGGER.info(f"Found {len(log_types)} log types with parsers. Starting pull...")
    ...
    def pull_parser(self, log_type: str):
        ...
        LOGGER.info(f"[{log_type}] Pulling active parser configuration from SecOps...")
        ...
        try:
            ...
            active_parser = self._get_active_content(log_type, is_extension=False, parser_type=ParserType.CUSTOM)
            parser_type = ParserType.CUSTOM
            
            if not active_parser:
                # Try PREBUILT
                active_parser = self._get_active_content(log_type, is_extension=False, parser_type=ParserType.PREBUILT)
                if active_parser:
        ...
        if active_ext:
            yaml_content["parser_extension"]["cbn_snippet"] = PARSER_EXT_CONFIG_FILENAME
             ...
             try:
                with open(parser_yaml_path, 'r', encoding='utf-8') as f:
                    ...
                    if existing_yaml:
                        ...
                        if "parser" not in existing_yaml: existing_yaml["parser"] = {}
                        ...
                        if "cbn" in existing_yaml.get("parser", {}):
                            yaml_content["parser"]["cbn"] = existing_yaml["parser"]["cbn"]
                        
                        # Only preserve extension filename if we have an active extension
                        if active_ext and "cbn_snippet" in existing_yaml.get("parser_extension", {}):
                            existing_name = existing_yaml["parser_extension"]["cbn_snippet"]
                            if existing_name == "parser_ext.conf":
                                yaml_content["parser_extension"]["cbn_snippet"] = PARSER_EXT_CONFIG_FILENAME
                            else:
                                yaml_content["parser_extension"]["cbn_snippet"] = existing_name
             except Exception as e:
                 LOGGER.warning(f"[{log_type}] Failed to read existing {PARSER_YAML_FILENAME}, overwriting: {e}")

        with open(parser_yaml_path, 'w', encoding='utf-8') as f:
            yaml.dump(yaml_content, f, sort_keys=False)
        LOGGER.info(f"[{log_type}] Updated '{PARSER_YAML_FILENAME}'.")

        parser_conf_filename = yaml_content.get("parser", {}).get("cbn", PARSER_CONFIG_FILENAME)
        parser_conf_path = os.path.join(parser_dir_path, parser_conf_filename)
        
        if active_parser:
            with open(parser_conf_path, 'w', encoding='utf-8') as f:
                f.write(active_parser)
            LOGGER.info(f"[{log_type}] Wrote active parser to '{parser_conf_path}'.")
        ...
        parser_ext_conf_filename = yaml_content.get("parser_extension", {}).get("cbn_snippet", PARSER_EXT_CONFIG_FILENAME)
        parser_ext_conf_path = os.path.join(parser_dir_path, parser_ext_conf_filename)
        
        if active_ext:
            with open(parser_ext_conf_path, 'w', encoding='utf-8') as f:
                f.write(active_ext)
            LOGGER.info(f"[{log_type}] Wrote active parser extension to '{parser_ext_conf_path}'.")
            ...
            LOGGER.info(f"[{log_type}] Removed local parser extension as none is active in SecOps.")

        # Create logs and events folders if they don't exist
        os.makedirs(os.path.join(parser_dir_path, LOGS_FOLDER_NAME), exist_ok=True)
        os.makedirs(os.path.join(parser_dir_path, EVENTS_FOLDER_NAME), exist_ok=True)
