...
class ParserComparator:
    ...
    def _read_git_file(self, filename: str, branch: str = "main") -> str | None:
        ...
        try:
            ...
            LOGGER.warning(f"Could not fetch {filename} from {branch}: {e.stderr.strip()}")
            return None
    ...
    def _flatten_single_event_dict(self, d, parent_key='', sep='.'):
        ...
        for k, v in d.items():
            ...
            if isinstance(v, dict):
                items.extend(self._flatten_single_event_dict(v, new_key, sep=sep).items())
            elif isinstance(v, list):
                for i, item in enumerate(v):
                    if isinstance(item, dict):
                        items.extend(self._flatten_single_event_dict(item, f"{new_key}[{i}]", sep=sep).items())
                    else:
    ...
    def _extract_all_flat_events(self, events_data):
        ...
        if isinstance(events_data, list):
            for entry in events_data:
                ...
                if isinstance(entry, dict) and "events" in entry and isinstance(entry["events"], list):
    ...
    def compare_content(self, old_parser: str, old_ext: str | None, new_parser: str, new_ext: str | None) -> str:
        ...
        results_new = generate_event_files(
            self.client, self.log_type, new_parser, new_ext, self.logs_dir, self.events_dir, file_suffix=""
        )
        results_old = generate_event_files(
            self.client, self.log_type, old_parser, old_ext, self.logs_dir, self.events_dir, file_suffix="_old"
        )
        ...
        for log_file in all_logs:
            ...
            if path_new and path_old:
                # Compare per file
                file_diffs = compare_yaml_files(path_old, path_new, ["eventTimestamp", "timestamp", "collectedTimestamp", "etag"])
                if file_diffs:
        ...
        if total_events_old != total_events_new:
            add_line(f"WARNING: Event count mismatch! (Diff: {total_events_new - total_events_old})")
        else:
            add_line("Event counts match.")

        if all_hierarchical_changes:
            add_line("-" * 60)
            add_line(f"Changed Fields ({len(all_hierarchical_changes)} detected across all logs):")
    ...
    def run(self, branch: str = None):
        ...
        LOGGER.info(f"Starting comparison for log type: {self.log_type} against {target}")
        ...
        if not parser_old and branch:
             LOGGER.info(f"No active parser found in SecOps. Falling back to git branch '{branch}'...")
        ...
        if not parser_old:
            LOGGER.error(f"Baseline configuration not found (checked SecOps Active and git '{branch}'). Cannot compare.")
            return f"Baseline configuration not found (checked SecOps Active and git '{branch}')."
            
        LOGGER.info(f"Comparing against {target}...")
        return self.compare_content(parser_old, parser_ext_old, parser_new, parser_ext_new)
