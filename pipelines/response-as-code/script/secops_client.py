...
class SecOpsClient:
    ...
    def __init__(self, customer_id: str, project_id: str, region: str):
        ...
        try:
            ...
            raise APIError(f"Failed to initialize Google AuthorizedSession: {e}") from e

    def _make_request(self, method: str, path: str, params: Dict[str, Any] = None, json_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """Makes an authenticated request to the Chronicle API."""
        url = f"{self.base_url}{path}"
        try:
            response = self._session.request(method, url, params=params, json=json_data)
            ...
            raise APIError(f"An unexpected error occurred during API request: {e}") from e
    ...
    def list_soc_roles(self) -> List[SocRole]:
        ...
        try:
            while True:
                ...
                response_json = self._make_request(method="GET", path=api_path, params=params)
    ...
    def get_playbooks(self) -> List[WorkflowMenuCard]:
        ...
        try:
            ...
            response_json = self._make_request(method="POST", path=api_path, json_data=payload)
            print(response_json)
    ...
    def get_playbook(self, identifier: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response_json = self._make_request(method="GET", path=api_path, params={"workflowIdentifier": identifier})
            return response_json
        except Exception as e:
            raise APIError(f"Failed to retrieve legacy workflow full info for {identifier}: {e}") from e
    ...
    def save_playbook(self, playbook: Workflow) -> None:
        ...
        response_json = self._make_request(method="POST", path=api_path, json_data=playbook.raw_data)
        return response_json
        LOGGER.info(
        ...
        try:
            ...
            if "environments" in response_json:
                for environment_data in response_json["environments"]:
                    environment_names.append(environment_data.get("displayName", ""))
        except Exception as e:
    ...
    def get_environment_group_names(self) -> List[str]:
        ...
        try:
            ...
            if "environmentGroups" in response_json:
                for environment_group_data in response_json["environmentGroups"]:
                    environment_group_names.append(environment_group_data.get("displayName", ""))
        except Exception as e:
                ...
                for category_data in response_json["payload"]:
                    workflow_categories.append(
                        WorkflowCategory(
                            ...
                            categoryState=category_data.get("categoryState", ""),
                            type=category_data.get("type", ""),
                        )
                    )
        except Exception as e:
            raise APIError(f"Failed to retrieve workflow categories: {e}") from e
