...
MARK_END = "<!-- END TFDOC -->"
MARK_OPTS_RE = re.compile(r"(?sm)<!-- TFDOC OPTS ([a-z_]+:\S+(?:\s+[a-z_]+:\S+)*)\s*-->")
...
Document = collections.namedtuple(
    "Document", "content files variables outputs recipes", defaults=[None]
)
...
def format_tfref(outputs, variables, files, fixtures, recipes=None, show_extra=False):
