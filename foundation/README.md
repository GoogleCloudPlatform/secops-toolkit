# Google SecOps Foundations

Setting up a production-ready Google SecOps tenant is often a time-consuming process. Google SecOps Foundations aims to speed up this process via two complementary goals. On the one hand, SecOps Toolkit Foundations automation seamlessly integrates with the customer GCP organization, whether this is a brand new organization or part of a larger enterprise one. Secondly, we provide a reference implementation of the SecOps Foundations design using best practices and Terraform, based on our experience with GCP customers.

Google SecOps Foundations comes from engineers in Google Cloud's Professional Services Organization, with a combined experience of years solving the typical technical problems faced by Google SecOps/Chronicle customers. While every organization has specific requirements, many common issues arise repeatedly. Solving those issues correctly from the beginning is key to a robust and scalable Google SecOps setup. It's those common issues and their solutions that Google SecOps Foundations aims to collect and present coherently.

Google SecOps Foundations includes many customization points making it an ideal blueprint for organizations of all sizes, ranging from startups to the largest companies for different SecOps deployment models (Single Tenant/Multi Tenant) and number of environments.
