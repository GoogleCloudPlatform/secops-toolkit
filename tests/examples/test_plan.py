...
def _test_terraform_example(plan_validator, example):
    ...
    with tempfile.TemporaryDirectory(prefix=prefix, dir=directory) as tmp_path:
        ...
        if example.type == "hcl":
            ...
            assets_path = (
                BASE_PATH.parent / str(example.module).replace("-", "_") / "assets"
            )
        ...
        summary = plan_validator(
            module_path=tmp_path, inventory_paths=inventory, tf_var_files=tf_var_files
        )
        ...
        result = subprocess.run(
            f"{binary} fmt -check -diff -no-color main.tf".split(),
            cwd=tmp_path,
