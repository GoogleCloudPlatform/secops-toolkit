...
SECOPS_TOOLKIT_ROOT = Path(__file__).parents[2]


def pytest_generate_tests(
    metafunc, test_group="example", filter_tests=lambda x: "skip" not in x
):
    """Find all README.md files and collect code examples tagged for testing."""
    if test_group in metafunc.fixturenames:
        readmes = SECOPS_TOOLKIT_ROOT.glob("**/README.md")
        examples = []
        ids = []
            ...
            for child in doc.children:
                if isinstance(child, marko.block.FencedCode):
                    ...
                    if child.lang in ("hcl", "tfvars"):
                        path = module.relative_to(SECOPS_TOOLKIT_ROOT)
