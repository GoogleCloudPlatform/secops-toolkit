...
def _prepare_root_module(path):
    ...
    with tempfile.TemporaryDirectory(dir=path.parent) as tmp_path:
        ...
        shutil.copytree(
            path, tmp_path, dirs_exist_ok=True, symlinks=False, ignore=ignore_patterns
        )
...
def plan_summary(
    ...
    **tf_vars,
    ...
    with _prepare_root_module(module_path) as test_path:
        ...
        extra_files = [
            (module_path / filename).resolve()
            for x in extra_files or []
            for filename in glob.glob(x, root_dir=module_path)
        ]
        extra_dirs = [(module_path / dirname).resolve() for dirname in extra_dirs or []]
        ...
        if "values" in inventory:
            try:
                validate_plan_object(
                    inventory["values"], summary.values, relative_path, ""
                )
    ...
    if isinstance(expected_value, dict) and isinstance(plan_value, dict):
        for k, v in expected_value.items():
            if v == "__missing__":
                ...
                validate_plan_object(
                    v, plan_value[k], relative_path, f"{relative_address}.{k}"
                )
...
def plan_validator_fixture(request):
    ...
    def inner(module_path, inventory_paths, basedir=None, tf_var_files=None, **tf_vars):
        if basedir is None:
            basedir = Path(request.fspath).parent
...
def e2e_validator(module_path, extra_files, tf_var_files, basedir=None):
    ...
    with _prepare_root_module(module_path) as test_path:
        ...
        tf_vars = {
            "prefix": f"{prefix}-{int(time.time())}{os.environ.get('PYTEST_XDIST_WORKER', '0')[-2:]}"
        }
        try:
            ...
            assert plan_before_state == plan_after_state, (
                "Plan not empty after apply for values"
            )
            ...
            assert plan_before_sensitive_state == plan_after_sensitive_state, (
                "Plan not empty after apply for sensitive values"
            )

            # If above did not fail, this should not either, but left as a safety check
            assert changes == {}, (
                f"Plan not empty for following resources: {', '.join(changes.keys())}"
            )
        finally:
...
def e2e_tfvars_path():
    ...
    if tfvars_path := os.environ.get("TFTEST_E2E_TFVARS_PATH"):
        ...
        with _prepare_root_module(
            _REPO_ROOT / "tests" / "examples_e2e" / "setup_module"
        ) as test_path:
            binary = os.environ.get("TERRAFORM", "terraform")
            tf = tftest.TerraformTest(test_path, binary=binary)
            tf_vars_file = None
            tf_vars = get_tfvars_for_e2e()
            tf_vars["suffix"] = os.environ.get("PYTEST_XDIST_WORKER", "0")[
                -2:
            ]  # take at most 2 last chars for suffix
