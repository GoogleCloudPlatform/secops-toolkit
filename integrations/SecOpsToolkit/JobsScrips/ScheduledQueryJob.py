...
class JobParametersParser:
    ...
    def __init__(self, siemplify_):
        ...
        self.user_id = JobParametersParser.parse_user_id(siemplify_)
...
def main():
    ...
    try:
        ...
        user_id = job_parameters_parser.user_id
        ...
        if not query_text:
            siemplify.LOGGER.error("Could not extract queryText from the retrieved search query. Aborting.")
            return
        ...
        siemplify.LOGGER.info(f"Triggering legacy fetch UDM search view from {start_time} to {end_time}")
        ...
        if not operation_name:
            error_msg = f"Search did not return an operation name. Response: {operation}"
        ...
        result_row_count = (
            completed_op.get("response", {})
            .get("metadata", {})
            .get("resultRowCount")
        )
        success_msg = (
            f"Search operation completed successfully with state: '{state}'."
        )
        if result_row_count is not None:
            success_msg += f" Materialized {result_row_count} rows."
        
        siemplify.LOGGER.info("Successfully triggered the data table population search.")
