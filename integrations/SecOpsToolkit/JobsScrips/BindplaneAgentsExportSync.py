...
class JobParametersParser:
    ...
    def __init__(self, siemplify_):
        ...
        self.bindplane_projects = JobParametersParser.parse_bindplane_projects(siemplify_)
        ...
        self.chronicle_customer_id = JobParametersParser.parse_chronicle_customer_id(siemplify_)
        self.chronicle_project_id = JobParametersParser.parse_chronicle_project_id(siemplify_)
    ...
    def parse_bindplane_projects(siemplify):
        ...
        try:
            ...
            siemplify.LOGGER.error("Failed to parse Bindplane Projects. Ensure it is a valid JSON dictionary.")
            return {}
    ...
    def parse_projects_metadata(siemplify):
        ...
        try:
            ...
            siemplify.LOGGER.error("Failed to parse Project Metadata. Ensure it is a valid JSON dictionary.")
            return {}
...
def fetch_agents_for_project(api_url, api_key, logger, limit=1000):
    ...
    while True:
        ...
        try:
            logger.info(f"Making request to {endpoint} with offset {offset} and limit {limit}")
...
def main():
    ...
    try:
        ...
        if not job_parameters_parser.bindplane_projects:
            siemplify.LOGGER.error("Bindplane Projects parameter is empty or invalid. Aborting.")
            return

        manager = GoogleBigQueryApiManager.from_config_params(job_parameters_parser.wif_email,job_parameters_parser.bq_project_id)
        ...
        for proj_name, api_key in job_parameters_parser.bindplane_projects.items():
            ...
            for agents_batch in fetch_agents_for_project(
                ...
                for agent in agents_batch:
                    ...
                    metadata = job_parameters_parser.projects_metadata.get(proj_name, {})
                    agent["region"] = metadata.get("region", "Unknown")
                    agent["country"] = metadata.get("country", "Unknown")
                    agent["status_ui"] = "Disconnected" if agent["status"] == 0 else "Connected"
        ...
        siemplify.LOGGER.info(f"Total agents collected and loaded across all projects: {total_loaded}")
        
        # SecOps Chronicle Data Table Sync
        if job_parameters_parser.chronicle_customer_id:
            siemplify.LOGGER.info("Initializing SecOps Chronicle Client to push agents data to Data Table...")
            try:
                ...
                try:
                    ...
                    siemplify.LOGGER.info(f"Data Table might already exist or creation returned an error: {e}")
                ...
                batch_size = 1000
                siemplify.LOGGER.info(f"Replacing Data Table rows for '{table_name}' with {len(dt_rows)} rows in batches of {batch_size}...")
                
                for i in range(0, len(dt_rows), batch_size):
                    batch = dt_rows[i:i + batch_size]
                    siemplify.LOGGER.info(f"Processing batch {i // batch_size + 1} ({len(batch)} rows)...")
                    chronicle.replace_data_table_rows(
                        name=table_name,
                        rows=batch
                    )
                siemplify.LOGGER.info("Successfully pushed agents to Chronicle Data Table.")
            except Exception as e:
