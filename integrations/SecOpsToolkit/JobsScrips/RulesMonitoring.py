...
class JobParametersParser:
    def __init__(self, siemplify_):
        self.api_root = extract_configuration_param(siemplify_, provider_name=INTEGRATION_NAME, param_name="API Root", is_mandatory=True, print_value=True)
        self.creds = extract_configuration_param(siemplify_, provider_name=INTEGRATION_NAME, param_name="User's Service Account", is_mandatory=False, print_value=False)
        self.verify_ssl = extract_configuration_param(siemplify_, provider_name=INTEGRATION_NAME, param_name="Verify SSL", default_value=True, input_type=bool, print_value=True)
        self.workload_identity_email = extract_configuration_param(siemplify_, provider_name=INTEGRATION_NAME, param_name="Workload Identity Email", print_value=False)
        
        # Email settings - split comma separated string into Python list
        recipient_emails_param = extract_action_param(siemplify_, "Recipient Emails", is_mandatory=True, print_value=True)
        self.recipient_emails = [email.strip() for email in recipient_emails_param.split(",") if email.strip()]

        # Tenant URL
        tenant_url_param = extract_action_param(siemplify_, "Tenant URL", is_mandatory=True, print_value=True)
        self.tenant_url = tenant_url_param.strip() if tenant_url_param else ""


def send_notification_email(parser: JobParametersParser, rules_to_notify: list, siemplify, logger):
    try:
        subject = 'Google SecOps - Rules Monitoring Notification'
        to_recipients = parser.recipient_emails
        tenant_url = parser.tenant_url
        tenant_link_html = f'<a href="{tenant_url}" target="_blank" style="color: #1a73e8; text-decoration: underline;">{tenant_url}</a>' if tenant_url else "Google SecOps Console"
...
def main():
    ...
    try:
        ...
        siemplify.LOGGER.info(f"Found {len(rules)} rules and {len(deployments)} deployments.")
        ...
        siemplify.LOGGER.info(f"Found {len(affected_rules)} rules in LIMITED or PAUSED state.")
        ...
        try:
            ...
            siemplify.LOGGER.info(f"Data table '{table_name}' does not exist. Will create it.")
        ...
        if data_table_exists:
            ...
            try:
                ...
                siemplify.LOGGER.info(f"Successfully created data table '{table_name}'.")
                ...
                siemplify.LOGGER.info(f"Successfully set TTL of 7 days for '{table_name}'.")
            except Exception as e:
        ...
        if rules_to_notify:
            siemplify.LOGGER.info(f"Notifying about {len(rules_to_notify)} new affected rules.")
            send_notification_email(parser, rules_to_notify, siemplify, siemplify.LOGGER)
            
            try:
                manager.create_data_table_rows(table_name, new_rows)
                siemplify.LOGGER.info(f"Successfully added {len(new_rows)} rows to '{table_name}'.")
            except Exception as e:
                siemplify.LOGGER.error(f"Failed to add rows to data table: {e}")
                
        else:
            siemplify.LOGGER.info("All affected rules were already notified recently. No email sent.")
