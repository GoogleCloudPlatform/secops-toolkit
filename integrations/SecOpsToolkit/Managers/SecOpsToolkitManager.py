...
class SecOpsToolkitManager:
    ...
    def validate_response(
        self, response: requests.Response, error_msg: str = "Request failed"
    ):
        try:
            response.raise_for_status()
        except requests.HTTPError as e:
            raise exceptions.SecOpsToolkitManagerError(
                f"{error_msg}: {e}. Response: {response.text}"
            )
    ...
    def clear_data_table(
        ...
        if logger:
            logger.info(
                f"Found {total_rows} rows in data table '{name}'. Deleting in batches of {batch_size}..."
            )
        ...
        if logger:
            logger.info(
                f"Successfully deleted all {total_deleted} rows from data table '{name}'."
            )
    ...
    def wait_for_operation(
        ...
        while time.time() - start_time < timeout_seconds:
            ...
            if logger:
                logger.info(
                    f"Operation {operation_name} status: done={is_done}, state={state}"
                )
    ...
    def list_rule_deployments(self, page_size: int = 1000) -> list:
        ...
        while data.get("nextPageToken"):
            ...
            self.validate_response(
                response, "Failed to list rule deployments (pagination)"
            )
            data = response.json()
            deployments.extend(data.get("ruleDeployments", []))
    ...
    def update_data_table(
        ...
        if "updateMask" in params and "row_time_to_live" in params["updateMask"]:
            params["updateMask"] = params["updateMask"].replace(
