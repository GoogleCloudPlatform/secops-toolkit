...
def assets_list(parent, asset_types, content_type, page_size, read_time,
                next_page=None):
...
def main(request):
  ...
  for chronicle_ingestion_label in chronicle_assets_config.keys():
    ...
    for node in nodes:
      ...
      more_results = True
      try:
        response = assets_list(node, asset_types, content_type, page_size,
                               lookback_timestamp)
        while more_results:
          ...
          if response.status_code == 200:
            ...
            if 'nextPageToken' in fetched_assets:
              ...
              LOGGER.info(f"nextPageToken: {fetched_assets['nextPageToken']}")
              response = assets_list(node, asset_types, content_type, page_size,
                                     lookback_timestamp,
                                     fetched_assets['nextPageToken'])
            else:
              if assets:
                LOGGER.info(f"{len(assets)} assets to be sent to PubSub.")
                for asset in assets:
                  send_to_pubsub(
                      pubsub_topic_id=chronicle_assets_config[
                          chronicle_ingestion_label]['pubsub_topic_id'],
                      asset=asset)
              else:
            ...
            LOGGER.info(f"Catch all for any other HTTP error codes.")
            more_results = False
            break
